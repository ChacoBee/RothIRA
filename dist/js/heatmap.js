(function () {
  ...
  const HEATMAP_INTEL_LIBRARY = {
    ...
    nasdaq100: {
      ...
      week: [
        "Xu hướng tuần phản ánh momentum công nghệ – phù hợp với sleeve GOOGL trong danh mục.",
  ...
  const HEATMAP_SLOW_LOAD_TIMEOUT_MS = 15000;
  const HEATMAP_HARD_FAIL_TIMEOUT_MS = 45000;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function clearLoadTimers() {
      if (slowLoadTimeoutId) {
        clearTimeout(slowLoadTimeoutId);
        slowLoadTimeoutId = null;
      }
      if (hardFailTimeoutId) {
        clearTimeout(hardFailTimeoutId);
        hardFailTimeoutId = null;
      }
    }
    ...
    function markWidgetLoaded(loadId) {
    ...
    function loadHeatmapWidget() {
      ...
      slowLoadTimeoutId = setTimeout(() => {
        ...
        slowLoadTimeoutId = null;
      }, HEATMAP_SLOW_LOAD_TIMEOUT_MS);

      hardFailTimeoutId = setTimeout(() => {
        if (!isActiveLoad(loadId)) {
          return;
        }
        if (widgetContainer.querySelector("iframe")) {
